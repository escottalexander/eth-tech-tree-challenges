...
```bash
    npx create-eth@1.0.2 -e BuidlGuidl/eth-tech-tree-challenges:dao-governance-proposals-and-voting-extension dao-governance-proposals-and-voting
```
...
```bash
  yarn account
```
To fund your account with testnet ETH (e.g., Sepolia), search for a faucet for your chosen network or ask around in onchain developer groups who are usually more than willing to share. Send the funds to your wallet address and run `yarn account` again to verify the balance appears for that network.
...
Once you have confirmed your balance on your chosen testnet (Sepolia shown below), you can run this command to deploy your contract.
Replace `sepolia` with your chosen network.
```bash
  yarn deploy --network sepolia
```
Now verify it on the block explorer for your chosen testnet (e.g., Sepolia Etherscan).
Replace `sepolia` with your chosen network.
```bash
